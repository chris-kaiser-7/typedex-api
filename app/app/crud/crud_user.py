...
class CRUDUser(CRUDBase[User, UserCreate, UserUpdate]):
    async def get_by_email(self, *, email: str) -> User | None: # noqa
    ...
    async def authenticate(self, *, email: str, password: str) -> User | None: # noqa
    ...
    async def validate_email(self, *, db_obj: User) -> User: # noqa
    ...
    async def activate_totp(self, *, db_obj: User, totp_in: NewTOTP) -> User: # noqa
        obj_in = UserUpdate(**UserInDB.model_validate(db_obj).model_dump())
        obj_in = obj_in.model_dump(exclude_unset=True)
        obj_in["totp_secret"] = totp_in.secret
        return await self.update(db_obj=db_obj, obj_in=obj_in)

    async def deactivate_totp(self, *, db_obj: User) -> User: # noqa
        obj_in = UserUpdate(**UserInDB.model_validate(db_obj).model_dump())
        obj_in = obj_in.model_dump(exclude_unset=True)
    ...
    async def update_totp_counter(self, *, db_obj: User, new_counter: int) -> User:  # noqa
        obj_in = UserUpdate(**UserInDB.model_validate(db_obj).model_dump())
        obj_in = obj_in.model_dump(exclude_unset=True)
        obj_in["totp_counter"] = new_counter
        return await self.update(db_obj=db_obj, obj_in=obj_in)

    async def toggle_user_state(self, *, obj_in: Union[UserUpdate, Dict[str, Any]]) -> User: # noqa
